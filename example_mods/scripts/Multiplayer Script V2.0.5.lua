...
---------------General Config---------------
playable = true		--change this to false if you want to play normally without multiplayer (or just move the script to another folder lol)
...
---------------Strum Config---------------
changeScroll = false		--if enabled lets you change the scroll type of each player (might not work with some modcharts)

downScrollP1 = true
downScrollP2 = false
--(if changeScroll is enabled the healthbar will become a vertical healthbar)
...
--advanced
--limit of the health drains (0 to 2)
drainLimitP1 = 1.8
drainLimitP2 = 0.2
...
function onUpdatePost(elapsed)
	...
	if playable then
		...
		if changeScroll and not (separatedHealthBars) then
			...
			--my attempt at a vertical healthbar (doesn't work that well)
			setProperty('iconP1.flipX', false)
			setProperty('iconP2.flipX', true)

			setProperty('iconP1.x', screenWidth - 134)
			setProperty('iconP2.x', screenWidth - 134)
			
			setProperty('iconP1.y', (getProperty('healthBar.y') - 26) - (getProperty('healthBar.percent') * 5.95) + 326)
			setProperty('iconP2.y', (getProperty('healthBar.y') - 26) - (getProperty('healthBar.percent') * 5.95) + 200)
...
function goodNoteHitP2(id, noteData, noteType, isSustainNote)
	...
	if getPropertyFromGroup('notes', id, 'hitByOpponent') == false and getPropertyFromGroup('notes', id, 'mustPress') == false then
